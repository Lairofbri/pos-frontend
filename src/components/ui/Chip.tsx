  ...
  return (
    <button
      ...
      className={`shrink-0 px-3.5 py-1.5 rounded-full text-sm font-body border-2 transition-all duration-200 cursor-pointer whitespace-nowrap ${
        active
          ? 'bg-pos-accent text-white border-pos-accent font-semibold'
          : 'bg-transparent text-text-secondary border-border hover:text-text-primary hover:border-pos-accent'
      }`}
    >
      {children}
    </button>
